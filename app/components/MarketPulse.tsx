              ...
              <div className='w-1/2 h-full flex flex-col justify-center items-center'>
                {tickers.length > 0 ? (
                  tickers.map((item) => (
                    <div
                      key={item.name}
                      className={`${item.name == "error: fetch failed" ? 'invisible' : 'visible'} flex text-md md:text-lg md:py-0 p-1 w-full h-1/6 sm:h-1/8 flex-row justify-between items-center rounded-full my-2 px-3 border-2 border-white`}
                    >
