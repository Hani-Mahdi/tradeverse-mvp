import React from 'react'
import Marquee from 'react-fast-marquee'
import he from "he"
import { NewsItem } from '../types/NewsItem'

const News = async () => {

  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/news?category=crypto`, {
    next: { revalidate: 3600 } 
  });

  if (!res.ok) {
    throw new Error(`Failed to fetch news: ${res.status}`);
  }

  const news: NewsItem[] = await res.json();

  return (
    <div className='w-screen text-white h-110 gradient-bg mt-40 mb-30'>
      <Marquee speed={50} className='w-screen h-110 border-black'>
        {news.length > 0 ? (
          news.map((item: NewsItem) => (
            <a href={item.url} key={item.id}>
              <div className='border-3 border-[#3A3A3A] bg-[#2A2A2A] h-90 w-100 mx-5'>
                <div className='ml-4 mt-4 w-full h-full'>
                  <h1 className='text-xl w-9/10 font-bold my-2'>{he.decode(item.headline)}</h1>
                  <h3 className='text-sm text-gray-200'>
                    {new Date(item.datetime * 1000).toUTCString()}
                  </h3>
                  <img src={item.image} className='w-3/4 h-2/5 my-2 bg-[#3A3A3A]' />
                  <h3 className="text-md w-9/10 text-gray-100 my-4">
                    {he.decode(item.summary.slice(0, 121))}...
                  </h3>
                </div>
              </div>
            </a>
          ))
        ) : (
          <p>No news found.</p>
        )}
      </Marquee>
    </div>
  )
}

export default News
